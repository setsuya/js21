document.addEventListener("DOMContentLoaded",function(){game=new blackjackGame();if(!game.saveGame("check")){game.saveGame("save");}else{game.saveGame("load");}
...
function listAllCards(){let all_cards="";cards.forEach((card)=>{let finished_card=printCard(card);all_cards+=finished_card;});document.getElementById("main").innerHTML=all_cards;}
function blackjackGame(){let game_info={"money":100,"hands_played":0,"hands_won":0,"hands_draw":0,"blackjacks":0};let panels={"waiting":`<span class="waiting-dice"><icon class="icofont icofont-dice"></icon> <icon class="icofont icofont-dice"></icon> <icon class="icofont icofont-dice"></icon> <icon class="icofont icofont-dice"></icon> <icon class="icofont icofont-dice"></icon></span>`,"results":`<button id="next-button" onclick="game.clearTable()">Next hand</button>`,"bet":`<input type="number" min="5" placeholder="$5 minimum bet" value="5" tabindex="1" /><button id="bet-button" onclick="game.checkBet(this)">Bet</button>`,"play":`<button id="hit-button" class="continue-button" onclick="game.dealCard([{'person': 'player', 'show': true}])">Hit</button><button id="stand-button" class="stop-button" onclick="game.dealerLoop()">Stand</button>`,"double_down":`<button id="double-down-button" onclick="game.doubleDown()">Double down</button><button id="pass-button" onclick="game.changeHUD('play')">Continue</button>`,"debug":`<button onclick="game.startGame()">Start</button><button onclick="game.clearTable()">Clear</button><button onclick="game.dealCard([{'person': 'player', 'show': true}])">Deal[player]</button><button onclick="game.dealCard([{'person': 'dealer', 'show': true}])">Deal[dealer]</button><button onclick="game.turnCard('dealer')">Turn card[dealer]</button><button onclick="game.countTotal('player')">Count[player]</button><button onclick="game.countTotal('dealer')">Count[dealer]</button>`}
let available_cards=Array();let dealer_total=0;let player_total=0;let bet_money=0;this.saveGame=function saveGame(action){function check(){if(localStorage.getItem("blackjackSave")===null){return false;}else{return true;}}
...
await this.sleep(900);fields[0].innerHTML="";fields[1].innerHTML="";document.getElementById("dealer").removeChild(banner_elements[0]);document.getElementById("player").removeChild(banner_elements[0]);this.countTotal("dealer");this.countTotal("player");this.checkDeck();}
...
this.checkBet=function checkBet(value){let bet=parseFloat(value.previousElementSibling.value);if(!isNaN(bet)&&bet>=5){this.updateGameInfo({"money":parseFloat(game_info.money-bet)});this.changeHUDInfo({"money":game_info.money.toFixed(2),"bet":bet.toFixed(2)});this.showChangeMoney("take",bet.toFixed(2));bet_money=bet;this.startGame();}}
this.doubleDown=function doubleDown(){this.updateGameInfo({"money":parseFloat(game_info.money-bet_money)});this.changeHUDInfo({"money":game_info.money.toFixed(2),"bet":(bet_money*2).toFixed(2)});this.showChangeMoney("take",bet_money.toFixed(2));bet_money+=bet_money;this.dealCard([{"person":"player","show":false}]);}
...
this.checkAccelerator=function checkAccelerator(key){let button_element;switch(key.code){case "KeyB":button_element=document.getElementById("bet-button");if(!key.altKey&&!key.metaKey&&button_element){button_element.click();}
...
break;case "KeyC":button_element=document.getElementById("pass-button");if(!key.altKey&&!key.metaKey&&button_element){button_element.click();}
break;}}
